<script lang="ts">
	...
	const departments = [
		...
		{ label: 'Engineering', count: null },
		{ label: 'Marketing', count: null },
		{ label: 'Sales', count: null },
		{ label: 'Human Resources', count: null },
		{ label: 'Operation', count: null }
	];

	const allReports = $derived(data.reports ?? []);
	const filtered = $derived(
		allReports.filter((r: any) => {
			const matchDept =
				selectedDepartment === 'All' || (r as any).department === selectedDepartment;
			const q = searchQuery.toLowerCase();
			const matchSearch =
				!q ||
				(r as any).reportNumber?.toLowerCase().includes(q) ||
		...
		<div class="filters">
			{#each departments as dept (dept.count)}
				<button
					...
					onclick={() => (selectedDepartment = dept.label)}
				>
				...
				<tbody>
					{#each filtered as row, i (row.id)}
						<tr class:selected={selectedRows.has(row.id)}>
							<td class="col-check">
								<button
									class="check-circle {selectedRows.has(row.id) ? 'checked' : ''}"
									onclick={() => toggleRow(row.id)}
								></button>
							</td>
							<td class="col-id">{row.reportNumber}</td>
							<td>{row.category}</td>
							<td>-</td>
							<td>
								<span class="badge {severityClass[row.severity] ?? 'sev-low'}">{row.severity}</span>
							</td>
							<td>
								<span class="badge {statusClass[row.status] ?? 'sta-new'}">
									{#if row.status === 'new'}
